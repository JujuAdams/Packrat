...
/// `new Packrat(width, height)`. A packrat struct has no public variables but many public methods
...
/// }
/// 
/// The box struct also contains a variable called "__index" which is for internal use and should
/// never be written to or read. Box structs cannot no methods.
/// 
...
function Packrat(_width, _height) constructor
{
    ...
    static DebugDraw = function(_x, _y, _scale)
    {
        ...
        repeat(array_length(_placedArray))
        {
            ...
            if (_box != undefined)
            {
                var _color = int64(ptr(string_copy(md5_string_utf8(string(_i*3254698)), 1, 6)));
